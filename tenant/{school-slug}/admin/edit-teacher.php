<?php
/**
 * School Edit Teacher Page
 * Handles editing existing teachers using TeacherManager class
 * 
...
ini_set('error_log', __DIR__ . '/../../../logs/school_edit_teacher.log');

error_log("=== EDIT TEACHER PAGE START ===");
...
if (!$teacherId) {
    $_SESSION['toast_error'] = "No teacher ID provided.";
...
if (!$teacher) {
    $_SESSION['toast_error'] = "Teacher not found.";
    header("Location: teacher-list.php");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit']) && $teacherManager) {
    ...
    error_log("=== PROCESSING TEACHER EDIT SUBMISSION ===");
    
    // Validate CSRF token
    if (!isset($_POST['csrf_token']) || !validateCsrfToken($_POST['csrf_token'])) {
        $_SESSION['toast_error'] = "Invalid security token. Please try again.";
        header("Location: " . $_SERVER['PHP_SELF'] . "?id=" . $teacherId);
        exit;
    }
    
    // Prepare teacher data (same structure as add)
    $teacherData = [
        'employee_id' => !empty($_POST['employee_id']) ? sanitize($_POST['employee_id']) : $teacher['employee_id'],
        ...
        'password' => $_POST['password'] ?? null, // only used if provided (optional)
        'profile_photo' => $teacher['profile_photo'] // keep existing unless new upload
    ];
    ...
    error_log("Teacher data prepared for update: " . json_encode(array_keys($teacherData)));
    ...
    // Update teacher using TeacherManager
    $result = $teacherManager->updateTeacher($teacherId, $teacherData);
    
    if ($result[0]) {
        $_SESSION['toast_success'] = $result[1];
        error_log("Teacher updated successfully: " . $result[1]);
        ...
        error_log("Failed to update teacher: " . $result[1]);
        $_SESSION['form_data'] = $_POST;
        header("Location: " . $_SERVER['PHP_SELF'] . "?id=" . $teacherId);
        exit;
    }
...
// Get form data from session for repopulation (after failed validation)
if (isset($_SESSION['form_data']) && !empty($_SESSION['form_data'])) {
    $formData = $_SESSION['form_data'];
    unset($_SESSION['form_data']);
} else {
    // Prefill with existing teacher data
    $formData = [
        ...
        'mothers_name' => $teacher['mothers_name'] ?? '',
        ...
        'bank_account_number' => $teacher['bank_account'] ?? '',
        ...
        'youtube_link' => $teacher['youtube_link'] ?? '',
        'details' => $teacher['details'] ?? '',
        'assigned_classes' => array_column($teacher['assigned_classes'] ?? [], 'id'),
        'assigned_subjects' => array_column($teacher['assigned_subjects'] ?? [], 'id')
    ];
}
...
error_log("=== EDIT TEACHER PAGE END ===");
?>
<!DOCTYPE html>
...
<head>
    ...
    <meta name="description" content="Edit Teacher - School Management System">
    <meta name="keywords" content="Edit Teacher, School Management">
    <meta name="robots" content="INDEX,FOLLOW">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Edit Teacher - <?php echo htmlspecialchars($school['name'] ?? 'School'); ?></title>
                    ...
                    <div class="shadow-1 radius-12 bg-base h-100 overflow-hidden">
                        ...
                        <div class="card-body p-20">
                            <div class="row gy-3">
                                <div class="col-sm-6">
                                    <div class="">
                                        <label for="password" class="text-sm fw-semibold text-primary-light d-inline-block mb-8">New Password</label>
                                        ...
                                        <small class="text-secondary-light">Only fill if you want to change the password</small>
                                    </div>
                                </div>
                                <div class="col-sm-6">
                                    <div class="">
                                        ...
                                        <div class="drop-zone height-44-px p-4 d-flex justify-content-center align-items-center text-center fw-medium text-md cursor-pointer border border-neutral-400 radius-8 border-dashed bg-hover-neutral-200">
                                            <span class="drop-zone__prompt">Drag & drop a new file here or click</span>
                                            <input type="file" name="profile_photo" class="drop-zone__input" accept="image/*">
                                        </div>
                                        <?php if (!empty($teacher['profile_photo'])): ?>
                                        <div class="current-photo">
                                            <p class="mb-1"><small>Current photo:</small></p>
                                            <img src="/<?php echo htmlspecialchars($teacher['profile_photo']); ?>" alt="Current profile photo">
                                        </div>
                                        <?php endif; ?>
                                    </div>
                                </div>
