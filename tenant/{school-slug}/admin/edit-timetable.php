...
/**
 * Edit Timetable Page
 * Allows admin to edit existing timetable entries
 * 
...
ini_set('error_log', __DIR__ . '/../../../logs/school_edit_timetable.log');

error_log("=== EDIT TIMETABLE PAGE START ===");
...
$timetableId = isset($_GET['id']) ? (int)$_GET['id'] : 0;
if (!$timetableId) {
    header("Location: timetable-list.php?error=Invalid timetable ID");
    exit;
}
...
// Get timetable data
$timetable = $timetableManager ? $timetableManager->getTimetable($timetableId) : null;
if (!$timetable) {
    header("Location: timetable-list.php?error=Timetable entry not found");
    exit;
}
...
$formData = array_merge($defaultFormData, is_array($timetable) ? $timetable : []); // Pre-populate with existing data
    ...
    try {
        ...
        if ($timetable['class_id']) {
            $sectionStmt = $schoolDb->prepare("
                ...
                ORDER BY name
            ");
            $sectionStmt->execute([$school['id'], $timetable['class_id']]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit']) && $timetableManager) {
    ...
    error_log("=== PROCESSING TIMETABLE UPDATE FORM ===");
    
    // Validate CSRF token
    if (!isset($_POST['csrf_token']) || !validateCsrfToken($_POST['csrf_token'])) {
        $toastError = "Invalid security token. Please try again.";
        $_SESSION['form_data'] = $_POST;
        header("Location: " . $_SERVER['PHP_SELF'] . "?id=" . $timetableId);
        exit;
    }
    ...
    error_log("Timetable update data prepared");
    ...
    // Update timetable using TimetableManager
    $result = $timetableManager->updateTimetable($timetableId, $timetableData);
    
    if ($result[0]) {
        $toastSuccess = $result[1];
        error_log("Timetable updated successfully: " . $result[1]);
        ...
        $toastError = $result[1];
        error_log("Failed to update timetable: " . $result[1]);
        $_SESSION['form_data'] = $_POST;
        header("Location: " . $_SERVER['PHP_SELF'] . "?id=" . $timetableId);
        exit;
    }
...
if (isset($_SESSION['form_data']) && !empty($_SESSION['form_data'])) {
    ...
    $postedFormData['academic_term_id'] = $postedFormData['academic_term_id'] ?? $postedFormData['academic_term'] ?? '';
    ...
    $postedFormData['subject_id'] = $postedFormData['subject_id'] ?? $postedFormData['subject'] ?? '';
    $postedFormData['teacher_id'] = $postedFormData['teacher_id'] ?? $postedFormData['teacher'] ?? '';
...
<head>
    ...
    <meta name="description" content="Edit Timetable - School Management System">
    <meta name="keywords" content="Edit Timetable, School Management">
    <meta name="robots" content="INDEX,FOLLOW">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Edit Timetable - <?php echo htmlspecialchars($school['name'] ?? 'School'); ?></title>
...
<main class="dashboard-main">
    ...
    <div class="dashboard-main-body">
        <div class="breadcrumb d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
            <div class="">
                <h1 class="fw-semibold mb-4 h6 text-primary-light">Edit Timetable Entry</h1>
                <div class="">
                    ...
                    <span class="text-secondary-light">/ Edit Timetable</span>
                </div>
            </div>
            <div>
                <span class="edit-badge me-2">ID: #<?php echo $timetableId; ?></span>
                <a href="timetable-list.php" class="btn btn-outline-primary d-inline-flex align-items-center gap-6">
            ...
            <div class="col-lg-8 mx-auto">
                <div class="shadow-1 radius-12 bg-base overflow-hidden">
                    <div class="card-header border-bottom bg-base py-16 px-24">
                        <h6 class="text-lg fw-semibold mb-0">Edit Timetable Information</h6>
                        <p class="text-secondary-light mb-0">Update the timetable entry details</p>
                    </div>
                    <div class="card-body p-24">
                        <form method="POST" action="" id="timetableForm">
                            <input type="hidden" name="csrf_token" value="<?php echo $csrfToken; ?>">
                            <input type="hidden" name="submit" value="1">
                            <input type="hidden" name="id" value="<?php echo $timetableId; ?>">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control form-select" id="academic_term" name="academic_term" required>
                                            ...
                                            <option value="<?php echo $term['id']; ?>" <?php echo ($formData['academic_term_id'] ?? '') == $term['id'] ? 'selected' : ''; ?>>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control form-select" id="day" name="day" required>
                                            ...
                                            <option value="<?php echo $day; ?>" <?php echo ($formData['day'] ?? '') == $day ? 'selected' : ''; ?>>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control form-select" id="period_number" name="period_number" required>
                                            ...
                                            <option value="<?php echo $num; ?>" <?php echo ($formData['period_number'] ?? '') == $num ? 'selected' : ''; ?>>
                                                Period <?php echo $num; ?>
                                            </option>
                                            <?php endforeach; ?>
                                        </select>
                                ...
                                <div class="col-12">
                                    <div class="form-check break-checkbox">
                                        <input class="form-check-input" type="checkbox" id="is_break" name="is_break" 
                                               <?php echo ($formData['is_break'] ?? 0) ? 'checked' : ''; ?>>
                                ...
                                <div class="col-12">
                                    <div class="d-flex align-items-center justify-content-center gap-3 mt-8">
                                        ...
                                        <button type="submit" class="btn btn-primary-600 border border-primary-600 text-md px-28 py-12 radius-8">
                                            <i class="ri-save-line me-2"></i>Update Timetable
                                        </button>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="mt-24 p-20 bg-neutral-50 radius-12">
                    <h6 class="fw-semibold mb-3"><i class="ri-information-line text-primary-600 me-2"></i>Update Guidelines</h6>
                    <ul class="text-secondary-light mb-0" style="font-size: 14px;">
                        ...
                        <li class="mb-2">✓ Changes will affect all students in this class</li>
                    </ul>
                </div>
            </div>
        </div>
                ...
                success: function(response) {
                    ...
                    if (response.success && response.sections.length > 0) {
                        $.each(response.sections, function(index, section) {
                            const selected = (section.id == <?php echo $formData['section_id'] ?? 0; ?>) ? 'selected' : '';
