...
function getPlanIcon($planName)
{
    ...
    if (strpos($name, 'enterprise') !== false) return 'fas fa-university';
    if (strpos($name, 'district') !== false) return 'fas fa-schools';
    if (strpos($name, 'growth') !== false || strpos($name, 'pro') !== false) return 'fas fa-chart-line';
...
<head>
    ...
    <title>Subscription Plans | <?php echo APP_NAME; ?> Admin</title>
...
<body>
                ...
                <form id="editPlanForm" method="POST" action="update_plan.php" onsubmit="return submitPlanForm(event)">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid md:grid-cols-4 gap-4">
                            ...
                            <div class="form-group">
                                <label class="form-label">Campus Limit *</label>
                                <input type="number" id="editCampusLimit" name="campus_limit" class="form-input" min="0" value="1" required>
                                <p class="text-xs text-slate-500 mt-1">0 = Unlimited</p>
                            </div>
    ...
    <div class="flex">
        <!-- Fixed Sidebar for Desktop -->
        <aside class="sidebar-desktop" id="sidebar">
                                                ...
                                                <td class="py-4">
                                                    <div class="space-y-1 text-xs">
                                                        ...
                                                        <div>
                                                            <span class="text-slate-600">Campuses:</span>
                                                            <span class="font-bold text-slate-900 ml-1">
                                                                <?php echo $plan['campus_limit'] == 0 ? '∞' : number_format($plan['campus_limit']); ?>
                                                            </span>
                                                        </div>
                                                        <div>
                                                            <span class="text-slate-600">Storage:</span>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Initialize slug generation with validation
            initSlugGeneration();

            // Format JSON features on blur
            const featuresTextarea = document.getElementById('editPlanFeatures');
            if (featuresTextarea) {
                featuresTextarea.addEventListener('blur', function() {
                    if (!formatJSON(this)) {
                        showNotification('Invalid JSON format in features', 'error');
                    }
                });
            }
        ...
        function initSlugGeneration() {
            ...
            if (nameInput && slugInput) {
                nameInput.addEventListener('input', function() {
                    // Only auto-generate if slug is empty or ends with copy
                    if (!slugInput.value || slugInput.value.match(/copy-\d+$/)) {
                ...
                slugInput.addEventListener('blur', function() {
                    ...
                    if (slug) {
                        // Ensure slug follows correct format
                        const validSlug = generateSlug(slug);
                        if (validSlug !== slug) {
                            this.value = validSlug;
                            showNotification('Slug automatically formatted', 'info');
                        }
                    }
                });
            }
        ...
        // Enhanced createNewPlan function
        function createNewPlan() {
            ...
            if (form) {
                ...
                document.getElementById('editStudentLimit').value = '50';
                document.getElementById('editTeacherLimit').value = '10';
                document.getElementById('editCampusLimit').value = '1';
                document.getElementById('editStorageLimit').value = '1024';
                document.getElementById('editPlanFeatures').value = JSON.stringify([
                    "Student Management",
                    "Attendance Tracking",
                    "Basic Reports",
                    "Email Support"
                ...
                // Generate slug from empty name
                document.getElementById('editPlanSlug').value = '';
        ...
        // Enhanced editPlan function
        async function editPlan(planId) {
            try {
                showNotification('Loading plan data...', 'info');
                ...
                if (result.success && result.plan) {
                    ...
                    document.getElementById('editCampusLimit').value = plan.campus_limit || 1;
                    ...
                    showNotification(result.message || 'Failed to load plan', 'error');
                }
            } catch (error) {
        ...
        // Enhanced confirmDeletePlan function
        async function confirmDeletePlan() {
            const planId = document.getElementById('deletePlanModal').getAttribute('data-plan-id');
            const planName = document.getElementById('deletePlanName').textContent;
        ...
        // Enhanced submitPlanForm function
        async function submitPlanForm(event) {
            ...
            try {
                ...
                if (result.success) {
                    const message = formData.get('plan_id') ? 'Plan updated successfully' : 'Plan created successfully';
        ...
        // Validate plan form before submission
        function validatePlanForm() {
            const name = document.getElementById('editPlanName')?.value.trim();
            const slug = document.getElementById('editPlanSlug')?.value.trim();
            ...
            const studentLimit = document.getElementById('editStudentLimit')?.value;
            const teacherLimit = document.getElementById('editTeacherLimit')?.value;
            const campusLimit = document.getElementById('editCampusLimit')?.value;
            
            const errors = [];
            
            if (!name) errors.push('Plan name is required');
            if (!slug) errors.push('Plan slug is required');
            if (!description) errors.push('Plan description is required');
            ...
            if (studentLimit) {
                const studentNum = parseInt(studentLimit);
                if (isNaN(studentNum) || studentNum < 0) {
                    errors.push('Student limit must be a valid non-negative number');
                }
            }
            
            if (teacherLimit) {
                const teacherNum = parseInt(teacherLimit);
                if (isNaN(teacherNum) || teacherNum < 0) {
                    errors.push('Teacher limit must be a valid non-negative number');
                }
            }
            
            if (campusLimit) {
                const campusNum = parseInt(campusLimit);
                if (isNaN(campusNum) || campusNum < 0) {
                    errors.push('Campus limit must be a valid non-negative number');
                }
            }
            ...
            if (features) {
                try {
                    ...
                    if (!Array.isArray(featuresArray)) {
                        errors.push('Features must be a JSON array');
                    }
                } catch (e) {
                    errors.push('Invalid JSON format in features');
                }
            }
        ...
        // Enhanced duplicatePlan function
        async function duplicatePlan(planId) {
            try {
                showNotification('Loading plan for duplication...', 'info');
                const response = await fetch(`get_plan.php?id=${planId}`);
                const result = await response.json();
                ...
                if (result.success && result.plan) {
                    ...
                    const copySlug = plan.slug + '-copy-' + Date.now();

                    document.getElementById('editPlanId').value = '';
                    document.getElementById('editPlanName').value = copyName;
                    document.getElementById('editPlanSlug').value = copySlug;
                    document.getElementById('editPlanDescription').value = plan.description;
                    document.getElementById('editPlanPrice').value = plan.price_monthly;
                    document.getElementById('editYearlyPrice').value = plan.price_yearly || '';
                    document.getElementById('editStudentLimit').value = plan.student_limit;
                    ...
                    document.getElementById('editIsDefault').value = '0'; // Never duplicate default flag
                    document.getElementById('editPlanStatus').value = '1'; // Always active by default
                    ...
                    openModal('editPlanModal');
                    updatePriceConversion();
                    showNotification('Plan ready for duplication', 'success');
                } else {
                    showNotification(result.message || 'Failed to load plan', 'error');
                }
            } catch (error) {
        ...
        // Helper function for JSON validation in modal
        ...
        function exportPlans() {
            const csvContent = "data:text/csv;charset=utf-8,Plan Name,Monthly Price,Yearly Price,Student Limit,Teacher Limit,Campus Limit,Storage,Status,Schools\n" +
                <?php foreach ($plans as $plan): ?> "<?php echo htmlspecialchars($plan['name']); ?>,$<?php echo $plan['price_monthly']; ?>,$<?php echo $plan['price_yearly']; ?>,<?php echo $plan['student_limit']; ?>,<?php echo $plan['teacher_limit']; ?>,<?php echo $plan['campus_limit'] ?? 1; ?>,<?php echo formatStorage($plan['storage_limit']); ?>,<?php echo $plan['is_active'] ? 'Active' : 'Inactive'; ?>,<?php echo $planStats[$plan['id']] ?? 0; ?>\n" +
                <?php endforeach; ?> "";
