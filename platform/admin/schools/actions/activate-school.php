...
try {
    ...
    $stmt = $db->prepare("UPDATE schools SET status = 'active', suspended_at = NULL WHERE id = ?");
    ...
    $logStmt = $db->prepare("
        ...
        VALUES (?, 'school_activated', ?, 'super_admin', NOW())
    ");
    $logStmt->execute([$schoolId, "School activated by super admin"]);
    ...
    error_log("Error activating school: " . $e->getMessage());
    echo json_encode(['success' => false, 'message' => 'Error activating school: ' . $e->getMessage()]);
}
?>